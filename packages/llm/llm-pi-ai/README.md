...
### Key pool (multiple credentials per route)

`apiKeyEnvs` lists further credential references that join `apiKeyEnv` as the route's **key pool**. Requests fan out across the pool and automatically retry on another account when one is rate-limited, quota-exhausted, rejected for authentication, or unreachable mid-request — so a deployment sits behind several accounts (of one provider or a shared gateway) without a load balancer and without coding rotation. Add an account by appending its credential reference to the list; remove one by deleting it. To spread load evenly, each request acquires a slot from the healthiest key using an adaptive scheduler with per-key state: a requests-per-minute token bucket and a tokens-per-minute budget that converge to each account's real sustainable rate from live outcomes (successes slowly raise the ceiling, `429`s and token-limit hits shrink it), a cooldown per failure class (a hard quota/auth failure is sent out of rotation for minutes; a transient rate limit pauses briefly), at most one in-flight request per key so concurrent calls fan out instead of bursting one account, and a minimum gap between request starts on the same key. A single-key route keeps today's behavior — `apiKeyEnv` alone is the whole pool — and a route naming no credential at all remains deliberately unauthenticated. `requestsPerMinute` seeds each key's initial per-request budget (default five); the scheduler still adapts from wherever it starts.

```yaml
      # One gateway, several accounts: requests rotate across them and retry
      # on the next account when one is rate-limited or quota-exhausted.
      gateway-pool:
...
`defaultModelThinking` breaks that "absent means non-reasoning" default at the route level: a dict of wire spellings (the same vocabulary a `reasoningEfforts` entry takes, from the provider's API documentation) is materialised as the reasoning capability of every model on the route that declares neither its own `reasoningEfforts` nor a reasoning catalog entry. A gateway whose models all support the same thinking dialect therefore needs no per-model `reasoningEfforts`; a catalog model that reasons and a model with explicit efforts both win over the fallback. The dict is validated like an entry's (an empty one or one with no level beyond `off` is refused; `false` is refused, since the route cannot disable a model's own reasoning).

Supported profile fields are `apiKeyEnv`, `apiKeyEnvs`, `requestsPerMinute`, `displayName`, `api`, `baseURL`, `models`, `modelOverrides`, `compat`, `defaultContextWindow`, `defaultMaxTokens`, `defaultInput`, `defaultModelThinking`, `headers`, `reasoning`, `thinkingBudgets`, `cacheRetention`, `transport`, `timeoutMs`, `websocketConnectTimeoutMs`, `streamIdleTimeoutMs`, `maxRequestImageBytes`, and `retryPolicy`. Each resolved profile retry policy is captured with that provider route; omission uses the shared bounded normal default of five retries. The stream-idle interval is a positive finite Node timer delay, defaults to five minutes, and covers only an outstanding provider read, not consumer think time. `apiKeyEnvs` extends the route's key pool (see "Key pool" above) and `requestsPerMinute` seeds each pooled key's initial request budget (default five). `defaultModelThinking` supplies the thinking levels of models on the route that declare neither their own `reasoningEfforts` nor a reasoning catalog entry (see "Per-model reasoning efforts" above). `maxRequestImageBytes` bounds one request's base64-encoded image payload (default 20MiB, a positive integer): every image in history is re-encoded into every request, so when the accumulated payload exceeds the bound, the oldest images are replaced by a fixed text placeholder until the request fits, keeping an image-heavy session serviceable instead of permanently rejected by a gateway request-size cap. The default leaves capacity for system prompts, history, tools, and JSON; deployments behind stricter gateways lower it per route. Harness app attribution wins a conflicting configured header name.
...
- **Key-pool rotation never switches a mid-response failure** — with multiple credentials, a terminal provider failure rotates to the next key only while no content chunk has reached the caller yet; once text, thinking, or tool-call deltas have been yielded, switching accounts mid-response would corrupt the conversation, so the failure is surfaced as-is. Scheduler state is per adapter instance and in-memory: a plugin reload resets every key's learned rate limits and cooldowns.
