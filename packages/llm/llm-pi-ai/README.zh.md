...
`apiKeyEnvs` 列出更多凭据引用，与 `apiKeyEnv` 一起组成该路由的**密钥池**。请求在池内分散，并在某个账号被限流、配额耗尽、认证被拒或请求中途不可达时自动换到另一个账号重试——因此部署可以同时使用多个账号（同一提供方或共享网关），无需负载均衡器，也不用自己写轮换逻辑。新增账号只需把它的凭据引用追加进列表；删除账号则删掉对应条目。为了均匀分摊负载，每个请求通过自适应调度器从最健康的密钥获取槽位——调度器为每个密钥维护独立状态：请求/分钟令牌桶与 token/分钟预算会根据真实结果收敛到各账号的实际可持续速率（成功缓慢上调上限，`429` 或命中 token 上限则下调），按失败类别冷却（硬性配额/认证失败会离开轮换数分钟，瞬时限流只短暂暂停），每个密钥最多一个在途请求（并发请求分配到不同账号，避免单个账号突发），同一密钥的相邻请求之间保留最小间隔。单密钥路由保持原有行为——仅 `apiKeyEnv` 本身就是整个池——完全不配置凭据的路由仍然刻意保持未认证。`requestsPerMinute` 为每个密钥设置初始每分钟请求预算（默认 5）；调度器仍会从该起点自适应调整。

```yaml
      # 一个网关、多个账号：请求在账号间轮换，某个账号被限流或配额
      # 耗尽时自动换到下一个账号重试。
      gateway-pool:
...
受支持的 profile 字段是 `apiKeyEnv`、`apiKeyEnvs`、`requestsPerMinute`、`displayName`、`api`、`baseURL`、`models`、`modelOverrides`、`compat`、`defaultContextWindow`、`defaultMaxTokens`、`defaultInput`、`defaultModelThinking`、`headers`、`reasoning`、`thinkingBudgets`、`cacheRetention`、`transport`、`timeoutMs`、`websocketConnectTimeoutMs`、`streamIdleTimeoutMs`、`maxRequestImageBytes` 和 `retryPolicy`。每条 profile 解析后的重试策略会随该提供方路由一同捕获；省略时使用共享的有界 normal 默认值并重试五次。流空闲间隔必须是正的有限 Node 定时器延迟，默认为五分钟，且只覆盖未完成提供方读取，不包括消费方思考时间。`apiKeyEnvs` 扩展路由的密钥池（见上方的"密钥池"一节），`requestsPerMinute` 为每个池内密钥设置初始请求预算（默认 5）。`defaultModelThinking` 为路由上那些既未声明 `reasoningEfforts`、也未具备推理能力 catalog 条目的模型提供思考级别（见上方的"单模型思考级别"一节）。`maxRequestImageBytes` 约束单个请求的 base64 编码图片载荷（默认 20MiB，正整数）：历史中的每张图片都会重新编码进每个请求，累积载荷超过上限时，从最老的图片开始替换为固定文本占位，直到请求装得下，使图片较多的会话保持可用，而不是被网关请求体上限永久拒绝。默认值为系统提示词、历史、工具与 JSON 保留请求容量；网关更严格的部署按路由调低该值。若已配置标头中有同名项，则以 Harness 应用归因为准。
...
- **密钥池轮换绝不切换中途失败的响应**：配置多个凭据时，只有在尚未向调用方产出任何内容块之前，终态提供方失败才会轮换到下一个密钥；一旦已经产出文本、思考或工具调用增量，中途切换账号会破坏对话，因此按原样上抛该失败。调度器状态按适配器实例保存在内存中：插件重载会重置每个密钥已习得的速率上限与冷却。
