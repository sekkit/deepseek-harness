...
import type { ChatNodeHookContext, ChatNodeOwnerProps, ChatViewSlotProps, RunningTurnActivity, UsePresentation } from '../contract/slots.ts'
...
interface ChatNodeSeatProps extends ChatNodeOwnerProps {
  ...
  /** Activity of the newest running Turn; only its own members forward it. */
  readonly runningTurnActivity: RunningTurnActivity
...
export const ChatNodeSeat = memo(function ChatNodeSeat({
  nodeKey, groupPart, runningTurnActivity, useChatNode, useChatNodeProcess, usePresentation,
  ...
  const owner = useMemo<ChatNodeOwnerProps | null>(() => node === undefined
    ...
    : {
      ...
      fileMentions,
      turnProcess,
      // A Turn's activity reaches only its own renderers: another Turn's
      // disclosure must not describe what this one is waiting on.
      ...runningTurnActivity === null || runningTurnActivity.turn !== turn
        ? {}
        : { turnActivity: runningTurnActivity },
    }, [
    node, groupPart, cwd, openFile, openSkill, inspectCall, forkAt,
    loadImage, renderMessageImages, fileMentions, turnProcess, runningTurnActivity, turn,
  ])
