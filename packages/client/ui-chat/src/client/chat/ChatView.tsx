...
import type {
  ConversationTimelineSnapshot, NodeKey, RenderEntry, RenderMessageImages, RunningToolCall,
 ...
 * @param timeline - turn/step facts of the Chat snapshot.
 * @param order - rendered Chat node keys in flow order.
 * @param nodes - materialized Chat nodes by key.
 * @returns the newest open Turn's activity, or null to keep the plain
 *   "Deep diving..." fallback.
 */
function turnActivity(
  ...
  if (runningTool !== undefined) {
    return { turn: openTurn.turn, key: 'chat.deepDiving.tool', params: { name: runningTool.name } }
  }
  if (streaming !== undefined) {
    ...
    if (hasOutput) return { turn: openTurn.turn, key: 'chat.deepDiving.outputting' }
    if (hasThinking) return { turn: openTurn.turn, key: 'chat.deepDiving.thinking' }
  }
  // The turn is open but nothing has streamed yet (first-token wait).
  return { turn: openTurn.turn, key: 'chat.deepDiving.waiting' }
}
...
export function ChatView({
  ...
  // Phase-aware activity for the running turn, re-derived on any snapshot
  // change — a cheap scan of the materialized Chat nodes and timeline.
  const turnActivityLabel = useMemo(
    () => running ? turnActivity(timeline, order, nodeStore) : null,
    [running, timeline, order, nodeStore],
  )
          ...
          <div ref={scroll.columnRef} className={css.column} data-chat-flow="">
            ...
            <MarkdownDelegateProvider openExternalLink={openExternalLink} openFile={requestOpenFile}>
              <ChatNodeList
                ...
                runningTurnActivity={turnActivityLabel}
                nodeStore={nodeStore}
