...
export const TurnProcessNodeView = memo(function TurnProcessNodeView({
  node, turnProcess, turnActivity, t,
  ...
  // What the running Turn is waiting on replaces the static label, so the
  // status names the tool, thinking, streaming output, or first-token wait.
  const activity = turnActivity === undefined ? t('chat.deepDiving') : t(turnActivity.key, turnActivity.params)
  // Other end reasons retain elapsed time; only cancellation and failure replace it.
  const label = running
    ? duration === undefined ? activity : t('message.turnProcess.deepDivingFor', { activity, duration })
  ...
  const announcement = running ? activity
    : reason === 'aborted' ? t('message.stopped')
      : reason === 'error' ? t('message.turnProcess.failed')
