...
Observed state is dropped on plugin disposal (HMR safety) and is never persisted across host restarts — a session resumed within the same process keeps its observations (keyed by the session's stable `id`); a full host restart starts with no observations.
